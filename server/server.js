    ...
    fetch(url, { signal: controller.signal })
      ...
      .then(omdbMovie => {
        if (omdbMovie.Response === 'True') {
          const convertedMovie = {
            ...
            Released: omdbMovie.Released !== "N/A" ? new Date(omdbMovie.Released).toISOString().split('T')[0] : null,
            Runtime: omdbMovie.Runtime && omdbMovie.Runtime !== "N/A" ? parseInt(omdbMovie.Runtime) : 0,
            Genres: omdbMovie.Genre && omdbMovie.Genre !== "N/A" ? omdbMovie.Genre.split(', ') : [],
            Directors: omdbMovie.Director && omdbMovie.Director !== "N/A" ? omdbMovie.Director.split(', ') : [],
            Writers: omdbMovie.Writer && omdbMovie.Writer !== "N/A" ? omdbMovie.Writer.split(', ') : [],
            Actors: omdbMovie.Actors && omdbMovie.Actors !== "N/A" ? omdbMovie.Actors.split(', ') : [],
            Plot: omdbMovie.Plot !== "N/A" ? omdbMovie.Plot : "",
            Poster: omdbMovie.Poster !== "N/A" ? omdbMovie.Poster : "",
            Metascore: omdbMovie.Metascore && omdbMovie.Metascore !== "N/A" ? parseInt(omdbMovie.Metascore) : 0,
            imdbRating: omdbMovie.imdbRating && omdbMovie.imdbRating !== "N/A" ? parseFloat(omdbMovie.imdbRating) : 0.0
          };
          ...
          req.session.save((err) => {
            if (err) {
              console.error("Session save failed after adding movie:", err);
              return res.sendStatus(500);
            }
            res.status(201).send(convertedMovie);
          });
        } else {
          res.sendStatus(404);
        }
      ...
      .catch((err) => {
        ...
        if (err.name === 'AbortError') {
          console.error('OMDb API movie fetch request timeout');
          return res.sendStatus(504);
        }
        console.error('OMDb API movie fetch error:', err);
        res.sendStatus(500);
      });
  } else {
...
app.get("/search", requireLogin, function (req, res) {
  ...
  fetch(url, { signal: controller.signal })
    ...
    .catch((err) => {
      clearTimeout(timeoutId);
      console.log("EXAKTER FEHLER BEI DER SUCHE:", err)
      if (err.name === 'AbortError') {
