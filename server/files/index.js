  ...
  fetch(`/movies/${imdbID}`, { method: 'PUT' })
    .then(response => {
      if (response.status === 201) {
        ...
        const resultItem = document.getElementById(`search-result-${imdbID}`);
        if (resultItem) {
          resultItem.remove();
        }
  ...
  fetch(`/search?query=${encodeURIComponent(query)}`)
    ...
    .then(results => {
      ...
      results.forEach(movie => {
        const resultContainer = new ElementBuilder("div")
        .id(`search-result-${movie.imdbID}`)
        .class("search-result-item");

        const displayYear = movie.Year ? ` (${movie.Year})` : '';
        new ElementBuilder("span")
        .text(`${movie.Title}${displayYear}`)
        .appendTo(resultContainer.element);

        new ElementBuilder("button")
        .text("Add")
        .listener("click", () => addMovie(movie.imdbID))
        .appendTo(resultContainer.element);

        resultContainer.appendTo(resultsDiv);
      });
...
window.onload = function () {
  ...
  document.getElementById('loginForm').addEventListener('submit', (e) => {
    ...
    .catch(error => {
      ...
      alert(error.message || messages.loginFailed);
    });
  });
